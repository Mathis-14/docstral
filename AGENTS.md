...
- Do not add module, class or function docstrings. Use explicit names and
comments only for non-obvious reasons; keep public tool descriptions explicit.
...
- Search Toolkit: [https://docs.mistral.ai/studio/search/search-toolkit](https://docs.mistral.ai/studio/search/search-toolkit)
- Search Toolkit quickstart: [https://docs.mistral.ai/studio/search/search-toolkit/quickstart](https://docs.mistral.ai/studio/search/search-toolkit/quickstart)
- Agentic Search: [https://docs.mistral.ai/studio/search/agentic-search](https://docs.mistral.ai/studio/search/agentic-search)
- Search Starter App: [https://github.com/mistralai/search-starter-app](https://github.com/mistralai/search-starter-app)
- Vibe CLI MCP servers: [https://docs.mistral.ai/vibe/code/cli/mcp-servers](https://docs.mistral.ai/vibe/code/cli/mcp-servers)
- Corpus sitemap: [https://docs.mistral.ai/sitemap.xml](https://docs.mistral.ai/sitemap.xml)
...
- D026 — Use one native `sync_page` activity call per canonical URL. Why:
  Mistral Workflows must retain each page's successful result and retry only
  unfinished page work. `discover_urls` reads sitemap seeds; deterministic
  workflow code deduplicates a bounded queue, follows returned internal links
  and redirects, then calls `plan_deletions` and one `delete_page` per removal
  only after reliable exploration. All I/O stays in activities, with input `{}`
  for `docstral-refresh`. A page activity downloads, extracts, compares its
  fingerprint, splits, embeds and replaces chunks. An unchanged page still
  returns links. A redirect to another identity returns that target before
  downloading it, allowing the workflow to deduplicate aliases. Vespa's `pages`
  collection stores page identity and the last confirmed fingerprint; clear
  that confirmation before replacement, then save it only after success.
